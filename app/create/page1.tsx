{/*serves as the original create component */}
...
export default function MemeCreator() {
  ...
  const [customRatios, setCustomRatios] = useState<string[]>(() => {
    if (typeof window !== 'undefined') {
      try {
        return JSON.parse(localStorage.getItem('customRatios') || '[]');
      } catch {
        return [];
      }
    }
  ...
  const getLayerAtPosition = (x: number, y: number): Layer | null => {
    for (let i = layers.length - 1; i >= 0; i--) {
      ...
      if (layer.type === 'image' && layer.width && layer.height) {
        ...
        const xs = layer.paths.map(p => p.x);
        const ys = layer.paths.map(p => p.y);
        const minX = Math.min(...xs), maxX = Math.max(...xs), minY = Math.min(...ys), maxY = Math.max(...ys);
        if (x >= minX && x <= maxX && y >= minY && y <= maxY) { return layer; }
  ...
  const getResizeHandle = (layer: Layer, x: number, y: number): string => {
    if (!layer.selected || layer.type === 'text') return '';
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (currentTool === 'crop' && cropRegion) {
      setCropRegion({ ...cropRegion, w: x - cropRegion.x, h: y - cropRegion.y });
      ...
      if (layer && layer.width && layer.height && layer.type !== 'text') {
  ...
  const handleMouseUp = () => {
    if (currentTool === 'crop' && cropRegion && selectedLayerId) {
      const layer = layers.find(l => l.id === selectedLayerId);
      if (layer && layer.type === 'image' && layer.imageData) {
        ...
        if (ctx) {
          const img = new Image();
          img.onload = () => {
            const canvasWidth = layer.width || 1;
            const canvasHeight = layer.height || 1;
            const scaleX = img.width / canvasWidth;
            const scaleY = img.height / canvasHeight;

            const startX = Math.min(cropRegion.x, cropRegion.x + cropRegion.w);
            const startY = Math.min(cropRegion.y, cropRegion.y + cropRegion.h);
            const cropWidth = Math.abs(cropRegion.w);
            const cropHeight = Math.abs(cropRegion.h);

            const relStartX = startX - layer.x;
            const relStartY = startY - layer.y;

            let sx = Math.max(0, relStartX * scaleX);
            let sy = Math.max(0, relStartY * scaleY);
            let sw = cropWidth * scaleX;
            let sh = cropHeight * scaleY;

            sw = Math.min(img.width - sx, sw);
            sh = Math.min(img.height - sy, sh);

            tmp.width = sw / scaleX;
            tmp.height = sh / scaleY;
  ...
  const addCustomRatio = (ratio: string) => {
    if (!ratio || typeof ratio !== 'string') return;
    const parts = ratio.split(':').map(p => String(Number(p)));
    if (parts.length !== 2 || Number(parts[0]) === 0 || Number(parts[1]) === 0) return;
    const normalized = `${parts[0]}:${parts[1]}`;
    if (![...aspectRatios.map(ar => ar.label), ...customRatios].includes(normalized)) {
      const newRatios = [...customRatios, normalized];
      setCustomRatios(newRatios);
      try { localStorage.setItem('customRatios', JSON.stringify(newRatios)); } catch {}
    }
    setAspectRatio(normalized);
  };

  const removeCustomRatio = (label: string) => {
    const newRatios = customRatios.filter(r => r !== label);
    setCustomRatios(newRatios);
    try { localStorage.setItem('customRatios', JSON.stringify(newRatios)); } catch {}
  ...
  return (
    <div className='min-h-screen animated-gradient-bg'  >{/*parent------*/}
    <div className="bg-white rounded-2xl shadow-lg max-w-6xl mx-auto" onClick={handleOutsideClick}>
      <div className="flex items-center justify-between p-3 border-b border-gray-200">
        <h2 className="text-lg font-medium text-gray-900">Create meme</h2>
        <div className="flex items-center gap-4">
          ...
          <button className="p-1.5 text-gray-400 hover:text-gray-600" onClick={() => router.push('/memes')}>
            <X size={18} />
          </button>
        </div>
      </div>
          ...
          <div className="space-y-4">
            ...
            >
              <div className="relative">
                ...
                />
                {cropRegion && currentTool === 'crop' && (
                  <div
                    className="absolute border-2 border-dashed border-red-500 pointer-events-none"
                    style={{
                      left: Math.min(cropRegion.x, cropRegion.x + cropRegion.w),
                      top: Math.min(cropRegion.y, cropRegion.y + cropRegion.h),
                      width: Math.abs(cropRegion.w),
                      height: Math.abs(cropRegion.h)
                    }}
                  />
                )}
        ...
        <div className="w-54 p-4 border-l border-gray-200">
          <div className="space-y-4">
            ...
            <div className="space-y-2 mt-8">
              ...
              <h4 className="text-gray-700 font-medium text-sm mt-4">Custom</h4>
              {customRatios.map(label => (
                <div key={label} className="flex items-center">
                  <button
                    onClick={() => setAspectRatio(label)}
                    className={`flex-1 text-left text-xs py-1 px-2 rounded transition-colors ${
                      aspectRatio === label ? 'bg-blue-100 text-blue-800' : 'text-gray-500 hover:bg-gray-100'
                    }`}
                  >
                    {label}
                  </button>
                  <button
                    onClick={() => removeCustomRatio(label)}
